...
survey_data <-
  survey_data %>%
  select(Constit_Code, Constit_Name, country,region, b01, b02, Age, education, y09)

# Drop observations outside from Scotland and country column

survey_data <- survey_data %>% filter(country != 2)
survey_data <- survey_data %>% select(-country)
...
survey_data$party <-
  ifelse(survey_data$party < 1 | survey_data$party == 11, NA, survey_data$party)
...
survey_data$spoil <- ifelse(survey_data$party == 12, 1, 0)
survey_data$other <- case_when(
  survey_data$conservative == 0 &
    survey_data$labour == 0 &
    survey_data$liberal_dem == 0 &
    survey_data$spoil == 0 ~ 1,
